...
class GuildMusicState:
    def __init__(self, loop):
        self.playlist = Playlist(maxsize=60)
    ...
    async def play_next_song(self, song=None, error=None):
        ...
        if self.playlist.empty():
            ...
            embed = discord.Embed(title="Now Playing :", description=source.info["title"])
            embed.set_thumbnail(url=source.info["thumbnail"])
            duration = str(datetime.timedelta(seconds=source.info["duration"]))
            embed.set_footer(text="Duration : {} Requester: {} Volume :{}".format(duration, source.requester,
                                                                                  int(source.volume * 100)))
...
class Music:
    def __init__(self, bot):
        ...
        self.Description = "Plays music from youtube"
        self.music_states = {}
    ...
    async def status(self, ctx):
        ...
        if ctx.music_state.is_playing():
            ...
            embed = discord.Embed(title="Now Playing :", description=song.info["title"])
            embed.set_thumbnail(url=song.info["thumbnail"])
            duration = str(datetime.timedelta(seconds=song.info["duration"]))
            embed.set_footer(
                text="Duration : {} Requester: {} Volume :{}".format(duration, song.requester, song.volume * 100))
    ...
    async def play(self, ctx, *, request: str):
        ...
        if not ctx.music_state.is_playing():
            ...
            embed = discord.Embed(title="Queued :", description=song.info["title"])
            embed.add_field(name="Position", value=ctx.music_state.playlist.qsize(), inline=True)
            embed.set_thumbnail(url=song.info["thumbnail"])
            duration = str(datetime.timedelta(seconds=song.info["duration"]))
            embed.set_footer(text="Duration : {} Requester: {}".format(duration, song.requester))
            await ctx.send(embed=embed)
